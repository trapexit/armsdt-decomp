...
| --- | --- | --- | --- | --- | --- | --- |
| `armlib` | confirmed | blocked | open | open | open | Step 1 compile-triage advanced: pointer-width blockers for archive base/list-tail globals were cleared (`DAT_0804d84c` retyped and `DAT_0804d7cc` traversal repaired), and wildcard-dir helpers now use pointer-safe context types with an explicit `FUN_0804ba90` prototype. Build remains blocked at link by missing `main` entry (`undefined reference to main`) after `rtk make`. Closure experiment: recover original startup wrapper by mapping `_start` -> `__libc_start_main(FUN_0804a988,...)` into a concrete C `main` shim (or proven equivalent), then rebuild and rerun two-way CLI parity tests. |
...
2. `armlib` step 1 core outputs remain present (`recompiled/Makefile`, `recompiled/src/`, `recompiled/tests/`), and compile triage is active with pointer-width state migration completed for this batch but link-stage startup recovery still required before first successful `recompiled/build/armlib` output.
