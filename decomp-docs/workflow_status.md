...
| --- | --- | --- | --- | --- | --- | --- |
| `armlib` | confirmed | blocked | open | open | open | Step 1 compile-triage remains blocked at runtime parity. This batch replaced the local `isgraph` import stub infinite loop with an ASCII classifier in `recompiled/src/armlib.c` and refreshed startup probes. Candidate `./build/armlib`, `./build/armlib -help`, and `./build/armlib -vsn` still time out (exit 124 under `timeout 5`) while original exits remain `1`/`0`/`1`; no-args still reaches the help path and prints only the initial banner section before hanging, and `-help`/`-vsn` still stop after reset breadcrumbs (`DBG:4a988:enter`, `DBG:4a988:post_b7f0`, `DBG:4a988:pre_reset`, `DBG:4a988:post_workspace_zero`, `DBG:4a988:post_globals_reset`). Next closure experiment: add focused checkpoints around `FUN_0804a378` print segments and `FUN_0804a458` option-error branches, then replace the first still-looping formatting/import stub reached on each path (likely `memset` for long `printf` formatting and/or `fprintf` for error diagnostics), and rerun no-args/help/vsn parity probes. |
