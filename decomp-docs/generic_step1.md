...
Use decompiled sources as reverse-engineering inputs to produce a compilable C99 implementation in `<tool>/recompiled/` that matches original behavior exactly for every input form, option path, output artifact, diagnostic, exit code, and edge case present in the original binary while reconstructing the likely original source structure. Remove decompiler artifacts and recover plausible original names, types, control flow, and data layout, but do not materially redesign algorithms, helper structure, or module boundaries unless strong evidence or a narrowly justified portability fix requires it. Treat reconstructed structure as likely original C89-era design while using C99 as the target language standard for the recompiled build. Start by copying `<tool>/<tool>_decomp_ghidra.c` into the recompiled source tree, commit that baseline, and then iterate until the result is compilable and fully functional.
...
- Treat step 1 as evidence-based reconstruction of likely original C89-era source design, not as general refactoring or modernization.
...
- Use renames, recovered structs, constants, and similar edits only to remove decompiler artifacts and move the code toward plausible original C89-era source form while keeping the recompiled target in C99.
...
- `<tool>/recompiled/Makefile`
- `<tool>/recompiled/src/` reconstructed source that targets C99 while aiming to preserve likely original C89-era names, types, and structure
...
- Avoid any cast chain that stores pointers through `int32_t` (or `s32`).
- Update function signatures when arguments or return values are pointer-like.
- Prefer fixed-width aliases (`u8`, `s8`, `u16`, `s16`, `u32`, `s32`, `u64`, `s64`) over `uintN_t`/`intN_t` when width is known; see the fixed-width integer type aliases section.
...
- Restructure only to remove decompiler artifacts and recover likely original C89-era source form.
- Preserve likely original function boundaries, control-flow shape, state-machine structure, data layout, and helper granularity when evidence supports them.
- Do not split, merge, or invent helper functions, abstractions, or modules unless strong evidence suggests the original source likely had them or a narrowly scoped portability fix requires it.
...
| --- | --- |
| `undefined1` / `byte` | `u8` (or `char`) |
| `undefined2` / `word` | `u16` or `s16` |
| `undefined4` / `dword` | `u32`, `s32`, or pointer |
| `undefined` | infer from usage and call signatures |
| `uint` | `u32` (or narrower fixed-width type if proven) |
| `ulong` | `u64`, `u32`, or `size_t` depending on semantics |
| `longlong` | `s64` |
| `code *` | explicit function-pointer typedef |

### Fixed-width integer type aliases

When an integer type is known to have a specific width from evidence (binary layout, format docs, objdump, or runtime behavior), prefer short fixed-width aliases over verbose `uintN_t`/`intN_t` spellings. Define them once in a shared header:
...
- Use these aliases only when the width is known from evidence (binary layout, format docs, objdump, or runtime behavior).
- Do not use aliases for pointer-width-dependent types; keep `size_t`, `ptrdiff_t`, `intptr_t`, and proper pointer types for those.
- Use unsigned forms (`u8`, `u16`, `u32`, `u64`) for raw bytes, flags, encodings, and on-disk format fields.
- Use signed forms (`s8`, `s16`, `s32`, `s64`) for values that participate in signed arithmetic or can be negative.
- Keep `bool` for boolean flags; do not alias it to a fixed-width integer.
- Apply these aliases consistently across `<tool>/recompiled/src/` and `<tool>/recreated/src/` once defined.
...
Apply these mechanical recovery edits once behavior is confirmed and the change moves the code toward plausible original C89-era source form while retaining C99 as the target language:
