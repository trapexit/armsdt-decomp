...
Source: [ARM Image Format (AIF-1993.txt)](https://www.chiark.greenend.org.uk/~theom/riscos/docs/CodeStds/AIF-1993.txt)
...
- **Executable AIF** (in which the header is part of the image) can be
  executed by entering the header at its first word. Code in the header ensures
  the image is properly prepared for execution before being entered at its
  entry address.
...
- The fourth word of a non-executable AIF image is the offset of its entry
  point from its base address. The most significant nibble of this word (in the
  target byte order) is `0x0`.
...
zero-initialised data. Compression and relocation are optional: AIF is often
used to describe very simple absolute images.
...
- read by the program loader from offset `0x28` in the file containing the AIF
  image;
- given by some other means, for example by instructing an operating system or
...
- relocate to load address (the image can be loaded anywhere and will execute
  where loaded);
- self-move up memory, leaving a fixed amount of workspace above, and relocate
  to this address (the image is loaded at a low address and will move to the
  highest address which leaves the required workspace free before executing
  there).
...
Self-Relocation Code", allowing AIF to be easily tailored to new environments.
Using this facility is described in "-AIF -Relocatable" starting on page 21 of
the User Manual.
...
For details of the format of the debugging tables see "ARM Symbolic Debug Table
Format".
...
Debugging data is absent unless the image has been linked using the linker's
`-d` option and, in the case of source-level debugging, unless the components
of the image have been compiled using the compiler's `-g` option.
...
providing a template for the AIF header in an area called `AIF_HDR` in the
...
re-startable (note: an AIF image is **not** reentrant). If an AIF image is to
be re-started then, following its decompression, the first word of the header
must be set to NOP. Similarly, following self-relocation, the second word of
the header must be reset to NOP. This causes no additional problems with the
...
commencing execution. The ARM cross-linker sets this field to NOP by default,
but it can be customised by providing your own template for the AIF header in
an area called `AIF_HDR` in the **first** object file in the input list to
`armlink`.
...
If the `<Address mode>` word has bit 8 set (`(address_mode & 0x100) != 0`),
then the image was linked with separate code and data bases (usually the data
is placed immediately after the code). In this case, the word at offset `0x34`
...
the list of relocations (which is terminated by `-1`). The code is entered via
a `BL` from the second word of the AIF header so, on entry, `r14 -> AIFHeader +
8`. In 26-bit ARM modes, `r14` also contains a copy of the PSR flags.
...
the image does not need to be moved, the code branches to `RelocateOnly`.
...
found. Here, a system service (`SWI 0x10`) is called to return the address of
the top of memory in `r1`. This is system specific and should be replaced by
whatever code sequence is appropriate to the environment.
...
You can customise the self-relocation and self-moving code generated by
`armlink` by providing your version of it in an area called `AIF_RELOC` in the
**first** object file in `armlink`'s input list.
