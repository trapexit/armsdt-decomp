...
Source: [ARM Symbolic Debug Table Format (ASDTF.txt)](https://www.chiark.greenend.org.uk/~theom/riscos/docs/CodeStds/ASDTF.txt)

## Acknowledgement

This design is based on work originally done for Acorn Computers Ltd. by Topexpress Ltd.

## Introduction

This document specifies the format of symbolic debugging data generated by ARM compilers, which is used by the ARM Symbolic Debugger (`armsd`) to support high level language oriented, interactive debugging.

For each separate compilation unit (called a `section`) the compiler produces debugging data, and a special `area` in the object code (see *ARM Object Format* for an explanation of ARM Object Format, including areas and their attributes). Debugging data are position independent, containing only relative references to other debugging data within the same section, and relocatable references to other compiler-generated areas.

Debugging data areas are combined by the ARM linker into a single contiguous section of a program image. For details of the ARM linker's capabilities see *The ARM Linker (armlink)* of the User Manual. For a description of the linker's principal output format see *ARM Image Format*.
...
Assembly-language level debugging is also supported, though in this case the debugging tables are generated by the linker. If required, the assembler can generate debugging table entries relating code addresses to source lines. Low-level debugging tables appear in an extra section item, as if generated by an independent compilation (see *Debugging Data Items in Detail*). Low-level and high-level debugging are orthogonal facilities, though `armsd` allows the user to move smoothly between levels if both sets of debugging data are present in an image.

## Terminology

A `byte` is 8 bits, usually considered unsigned. A `word` is 32 bits (4 bytes), often considered signed. A `half word`, also called a `short`, is 16 bits (2 bytes). Half words are unused, except in the long form of `LineInfo` items.
...
The flag bytes are followed by these word-sized fields:
...
fileinfo    offset in the debugging area of the fileinfo item for this section (0 if absent)
debugsize   total byte length of debug data for this section
name/nsyms  string or integer
```
...
debugsize   total size of the low-level debugging data including this section item
```

For linker-generated debugging data, the section item is followed by `nsyms` `symbol` items, each consisting of 2 words:
...
`sym` encodes an index into the string table in the 24 least significant bits, and these flag values in the 8 most significant bits:

```text
ASD_GLOBSYM   0x00000000  if the symbol is absolute
ASD_ABSSYM    0x01000000  if the symbol is global
ASD_TEXTSYM   0x02000000  if the symbol names code
ASD_DATASYM   0x04000000  if the symbol names data
ASD_ZINITSYM  0x06000000  if the symbol names 0-initialised data
```

Note that the linker reduces all symbol values to absolute values, so the flag values record the history, or origin, of the symbol in the image.
...
endproc    offset/pointer to the related endproc item
fileentry  offset/pointer to file list entry for source file
name       string
```
...
A label in a source program is represented by a special procedure item with no matching endproc (`endproc` field is `0` to denote this). Pascal and Fortran numerical labels are converted by their respective compilers into strings prefixed by `$n`.
...
fileentry  offset/pointer to the file-list entry for the procedure's end
...
location      meaning depends on storage class
name          string
```

The following codes define storage classes:
...
The `location` field depends on storage class: absolute address for static/external variables (relocated by linker); stack offset (from frame pointer) for automatic and var-type arguments; argument-list offset for Fortran arguments; register number for register variables (floating-point registers are numbered `16..23`).

No account is taken of variables which ought to be addressed by positive offsets from the stack-pointer rather than negative offsets from the frame-pointer.
...
A struct item is used to describe a structured data type (e.g. a `struct` in C or a record in Pascal). After its code and length field, a struct item contains:

```text
fields        number of fields in the structure
size          total byte size of the structure
fieldtable... array of <fields> struct field items
```

Each struct field item contains:
...
C bit fields are not treated in full detail: a bit field is represented as an integer starting on the appropriate word boundary (so that the word contains the whole field).

### Array Items

An array item is used to describe a one-dimensional array. Multi-dimensional arrays are described as arrays of arrays. Which dimension comes first is source-language dependent (different for C and Fortran). After its code and length field, an array item contains:
...
If the `size` field is zero, debugger operations affecting the whole array (rather than individual elements) are forbidden.
...
A bound is undefined when no information about it is available.

A bound is constant when its value is known at compile time; in this case, the corresponding bound field gives its value.

If a bound is variable, the offset field identifies a variable debug item describing the location containing the bound. In an object file, the field contains byte offset from start of debug area to the variable item; in memory it contains a pointer to the corresponding variable item. A variable item may describe a location known to the compiler even when it does not correspond to a source-language variable.

### Subrange Items

A subrange item is used to describe a subrange type in Pascal. It also serves to describe enumerated types in C, and scalars in Pascal (in which case the base type is understood to be an unsigned integer of appropriate size). After its code and length field, a subrange item contains:
...
The `sizeandtype` field encodes byte size of container for the subrange (`1`, `2`, or `4`) in its least significant 16 bits, and a simple type code (see *Representation of Data Types*) in its most significant 16 bits. The type code refers to the base type of the subrange.

Example: a subrange `256..511` of unsigned short might be held in 1 byte.
...
An enumeration item describes a Pascal or C enumerated type.

After its code and length word, a **contiguous enumeration** contains:

```text
type      a type word describing the container type
...
A **discontiguous enumeration** (such as `enum bits {bit0=1, bit1=2, bit2=4, bit3=8, bit4=16}`) contains:
...
Each nametable entry has variable length:

```text
val   the enumerated value (e.g. 1/2/4/8/16)
name  the name of the enumerated element
```

### Function Declaration Items

After its code and length word, a function declaration item contains:

```text
type      a type word describing return type
argcount  number of arguments
args      sequence of <argcount> argument description items
```

Each argument description item contains:

```text
type  a type word describing argument type
name  argument name (may be several words)
```
...
These items mark beginning and end of a naming scope and must be properly nested in the debug area.

In each case, after code and length word, there is one word-sized field:

```text
codeaddress  address of start/end of scope (as determined by code word)
```

### Bitfield Items

A bitfield item describes an individual bitfield member of a C structure. After its code and length word, it contains:

```text
type       a type word describing the bitfield type
container  a type word describing the container type
size       a byte giving bitfield size (bits)
offset     a byte giving bitfield offset within container
zero       2 zero bytes
```
...
Each source file is described by a sequence of `fragments`. Each fragment describes a contiguous region of the file, within which addresses of compiled code increase monotonically with source file position. Fragment order is not necessarily related to source-file positions.

For compilations without `#include`, the fragment list may have one entry and all line-number information can be contiguous.

After its code and length word, the fileinfo item is a sequence of file entry items:

```text
len           length of this entry in bytes (including fragments)
date          last-modified date/time (0 if not available or unused)
filename      string (or "" if unknown)
fragment data see below
```

If present, `date` is seconds since 1970-01-01 (Unix epoch).

Following the final file entry item is a single `0` word marking end of sequence.

The fragment data is a word giving number of following fragments followed by those fragment items:
...
Each fragment item consists of 5 words, followed by lineinfo data:

```text
size       length of this fragment in bytes (including lineinfo items)
firstline  line number
lastline   line number
codestart  pointer to start of fragment's executable code
codesize   byte size of code in fragment
lineinfo   variable-length mapping of line numbers to code addresses
```

Each lineinfo item describes a source statement and consists of a pair of unsigned bytes, possibly followed by two or three unsigned half words (half words use target-endian byte order).

Short form (byte pair):
...
`lineinc` rules for next statement position from current `(line, column)`:

- If `0 <= lineinc < 64`, next is `(line + lineinc, 1)`.
- If `lineinc >= 64`, next is `(line, column + lineinc - 64)`.

A statement may generate zero bytes of code if line increment is non-zero (e.g. block start/end).

It is not possible to describe a statement with both no code and no line increment in short form, because that encoding is an escape to long-form lineinfo.

If `codeinc > 255`, or `lineinc` must represent line change `> 63` or column change `> 191`, both bytes are written as zero increments and real values are stored in following two or three half words.

Two-half-word form (next statement starts at column 1):
...
Note: in half-word forms, order of `lineinc` and `codeinc` is reverse of byte-pair order.

Three-half-word form (next statement starts at column != 1):

```text
escape   encoded as codeinc = 0, lineinc = 64 in byte pair
lineinc  source lines occupied by this statement
codeinc  bytes of code generated by this statement
newcol   absolute starting column for next statement
```

`newcol` is an absolute column, not an increment.

This lineinfo encoding is an incompatible change from previous format (version 2). In that older format, `lineinc` in a two-byte item always describes a line increment, and there is no four-half-word equivalent. Programs interpreting ASD tables should interpret lineinfo items according to the table format version in the section item.
