...
If the target function is reentrant (see `APCS Variants`) then it has
two entry points and control arrives:
...
unit is not specified by the APCS. See `ARM Shared Library Format` of
the Reference Manual for details of support for reentrant code and
shared libraries).
...
block entry or passed as o`pen array arguments,` the size of which is
